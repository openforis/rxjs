...
describe('Observable.prototype.concatMap', function () {
    ...
    it('should support the deprecated resultSelector', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.concatMap(function (x) { return rxjs_1.of(x, x + 1, x + 2); }, function (a, b, i, ii) { return [a, b, i, ii]; }))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should support a void resultSelector (still deprecated)', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.concatMap(function (x) { return rxjs_1.of(x, x + 1, x + 2); }, void 0))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should handle a never source', function () {
        testScheduler.run(function (_a) {
            ...
            var innersubs = [];
            var expected = '  -';
            var result = e1.pipe(operators_1.concatMap(function () {
                return inner;
            }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(inner.subscriptions).toBe(innersubs);
